...
$sql->command('select')->source({name => 'foo', as => 'boo'})->columns(qw/ foo bar /);
is("$sql", 'SELECT `foo`, `bar` FROM `foo` AS `boo`');

$sql->command('select')->source('table1')->columns('foo')->source(
    {   join       => 'inner',
        name       => 'table2',
...
$sql->command('select')->source('table1')->source('table2')->source(
    {   join       => 'inner',
        name       => 'table3',
        constraint => 'table1.foo=table2.bar'
    }
